    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = NudgeViewModel(screenUUID: uuid)
                    let window = createNudgeWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createNudgeWindow(for: selectedScreen, with: vm)
            }
