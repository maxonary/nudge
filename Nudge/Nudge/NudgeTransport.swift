...
final class NudgeTransport: ObservableObject {
    ...
    func send(to recipient: String, from sender: String, message: String? = nil) async throws {
        ...
        req.httpMethod = "POST"
        req.setValue("\(sender) wants you", forHTTPHeaderField: "Title")
        // The sender lives in the Title header. The body carries the optional
        // free-text message; when there's no message we fall back to the sender
        // name so older clients still surface something sensible.
        let trimmed = message?.trimmingCharacters(in: .whitespacesAndNewlines)
        let body = (trimmed?.isEmpty == false) ? trimmed! : sender
    ...
    private func subscribeOnce(user: String) async throws {
        ...
        for try await line in bytes.lines {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                ...
                // Sender is encoded in the title as "{sender} wants you".
                let suffix = " wants you"
                var sender = "Someone"
                if title.hasSuffix(suffix) {
                    let parsed = String(title.dropLast(suffix.count))
                    if !parsed.isEmpty { sender = parsed }
                } else if !body.isEmpty {
                    sender = body
                }
                // The body holds the optional message — unless it's just the
                // sender name (the no-message fallback).
                let trimmedBody = body.trimmingCharacters(in: .whitespacesAndNewlines)
                let message: String? = (trimmedBody.isEmpty || trimmedBody == sender)
                    ? nil : trimmedBody
                await MainActor.run {
                    self.lastIncoming = NudgePing(sender: sender, message: message, receivedAt: Date())
                }
            }
        }
    }
